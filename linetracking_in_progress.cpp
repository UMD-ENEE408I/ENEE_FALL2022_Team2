...
void setup() {
  ...
  if(connected)
  {
    ...
    while(!readTimes){
      Serial.println("HERE");
      packetSize = udp.parsePacket();
      udp.read((char*)times, sizeof(times));
      Serial.println("HERE2");
      readTimes = true;
      for(int i = 0; i < numTimes; i++){
        Serial.printf("i is %d \n", i);
        Serial.printf("size of times is %d\n", sizeof(times)/sizeof(float));
        Serial.printf("times is %d\n", times);
        Serial.printf("times[i] is %f\n", times[i]);
        float ti = 4.0;
        Serial.printf("ti is %f\n",ti);
        Serial.printf("HERE6\n");
        boolean ti0 = ti == 0;
        Serial.printf("ti0 is %B\n", ti0);
        if(times[i] == 0.0){
